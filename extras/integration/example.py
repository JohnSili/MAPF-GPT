...
def main():
    ...
    parser.add_argument('--weights', type=str, default=None,
                        help='Path to a .pt checkpoint (overrides --model, e.g. trimmed weights)')
    parser.add_argument(
        '--dtype',
        type=str,
        choices=['float32', 'bfloat16', 'float16'],
        default='float32',
        help='Model activations/weights dtype on device (non-quantized checkpoints only; default: float32)',
    )
    ...
    path_to_weights = args.weights if args.weights else f'weights/model-{args.model}.pt'
    weights_label = Path(path_to_weights).stem if args.weights else args.model
    ...
    svg_path = f"svg/{args.map_name}-{weights_label}-seed-{args.seed}.svg"
