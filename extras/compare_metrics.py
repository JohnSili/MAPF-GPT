...
Usage (from repository root):
  python extras/compare_metrics.py
  python extras/compare_metrics.py --map_name validation-mazes-seed-000 --num_agents 32 --seed 0
"""
...
RUNS = [
    ...
    ("2M-L3 (trim, ft)", "weights/model-2M-L3-finetuned.pt", "cuda"),
    ("2M-L4 (trim)", "weights/model-2M-L4.pt", "cuda"),
    ("2M-L4 (trim, ft)", "weights/model-2M-L4-finetuned.pt", "cuda"),
...
def plot_metrics(rows: list[dict], out_path: Path, map_name='одна сцена') -> None:
    ...
    fig.suptitle(f"MAPF-GPT: {map_name}", fontsize=13)
...
def main():
    ...
    plot_metrics(rows, args.output, args.map_name)
